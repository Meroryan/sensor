...
def main():
	...
	while 1:
		...
		for dev in devices:
			...
			if (AdvDataStr != None) and (AdvDataStr != "CC-RT-BLE"):
				mac = dev.addr
				...
				client.publish(mac +"/output/temperature", TempFloat)
				client.publish(mac +"/output/humidity", HumFloat)
				client.publish(mac +"/output/pressure", PressFloat)
				client.publish(mac +"/output/quality", float(Quali))
				client.publish(mac +"/output/illumination", float(Illu))
				client.publish(mac +"/output/soilhumidity", float(SoilHum))
				client.publish(mac +"/output/battery", float(Battery))
